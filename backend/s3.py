...
def save_state(state):
    s3.put_object(Body=json.dumps(state), Bucket='soham-govande', Key='d3n/state.json', ContentType='text/plain')

def create_patch(patch: str):
    # md5 hash of the patch
    patch_hash = hashlib.md5(patch.encode('utf-8')).hexdigest()
    # save the patch to s3
    s3.put_object(Body=patch, Bucket='soham-govande', Key=f'd3n/{patch_hash}.patch', ContentType='text/plain')
